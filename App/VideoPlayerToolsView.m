...
-(UILabel *)lTime{
    ...
    if (!_lTime) {
        _lTime = [UILabel new];
        _lTime.frame = CGRectMake(CGRectGetMaxX(_progressSr.frame) + 10, 0, self.frame.size.width - CGRectGetWidth(_progressSr.frame) - CGRectGetWidth(_bCheck.frame), self.frame.size.height);
...
-(UISlider *)progressSr{
    ...
    if (!_progressSr) {
        ...
        _progressSr.frame = CGRectMake(CGRectGetMinX(_bufferPV.frame) - 10, CGRectGetMidY(_bufferPV.frame) - 10, CGRectGetWidth(_bufferPV.frame) - 4, 20);
...
-(UIProgressView *)bufferPV{
    ...
    if (!_bufferPV) {
        ...
        _bufferPV.frame = CGRectMake(CGRectGetMaxX(_bCheck.frame), CGRectGetMidY(_bCheck.frame) - 2, self.frame.size.width-self.frame.size.height-100, 4);
